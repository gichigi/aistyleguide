...
export async function loadTemplate(templateName: string): Promise<string> {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/load-template?name=${templateName}`
    );
...
export async function processTemplate(
  ...
  try {
    ...
    try {
      ...
      const voiceTraitsResult = await generateBrandVoiceTraits(
        validatedDetails
      );
      ...
      if (!voiceTraitsResult.success) {
        throw new Error(
          `Failed to generate voice traits: ${voiceTraitsResult.error}`
        );
      }
      ...
      if (!validateMarkdownContent(content)) {
        throw new Error(
          "Generated voice traits do not match required markdown format"
        );
      }
    ...
    try {
      ...
      for (const section of sections) {
        const ruleResult = await generateStyleGuideRules(
          validatedDetails,
          section
        );
        if (!ruleResult.success) {
          throw new Error(
            `Failed to generate rules for ${section}: ${ruleResult.error}`
          );
        }
        ...
        if (!validateMarkdownContent(content)) {
          throw new Error(
            `Generated rules for ${section} do not match required markdown format`
          );
        }
    ...
    if (plan === "complete") {
      try {
        ...
        if (!exampleResult.success) {
          throw new Error(
            `Failed to generate examples: ${exampleResult.error}`
          );
        }
        ...
        if (!validateMarkdownContent(content)) {
          throw new Error(
            "Generated examples do not match required markdown format"
          );
        }
